...
vi.mock('../../../src/utils/fetch.ts', async (importOriginal) => {
  ...
  return {
    ...
    fetchWithCache: vi.fn().mockImplementation((url, init) => {
      ...
      return actual.fetchWithCache(url, init);
    }),
  };
});
  ...
  for (const file of exampleFiles) {
    it(`should render ${file}`, async () => {
      ...
      if (existsSync(goldenPath)) {
        ...
        expect(result).toBe(goldenSvg);
      } else {
        writeFileSync(goldenPath, result);
